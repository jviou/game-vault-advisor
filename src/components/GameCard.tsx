...
interface GameCardProps {
  ...
  onView?: (game: GameDTO) => void; // 👈 optionnel
}
    ...
    <div className="group relative bg-gradient-card border border-border rounded-xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm text-muted-foreground ml-2">{(game.rating ?? 0)}/5</span>
        </div>
        ...
        {!!game.genres?.length && (
          <div className="flex flex-wrap gap-1">
            ...
            {game.genres.length > 3 && (
              <Badge variant="outline" className="text-xs">+{game.genres.length - 3}</Badge>
            )}
          </div>
        )}
