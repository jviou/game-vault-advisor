...
export default function SagaPage() {
  ...
  async function refresh() {
    try {
      ...
      const only = data.filter((g) => normalizeSaga(g.saga) === sagaCanonical);
      // tri order puis createdAt (l'ordre affiché définit le “premier” pour la cover)
      only.sort((a, b) => {
  ...
  // ------- Actions de saga (ENTÊTE) -------

  // 1) Renommer (met à jour tous les jeux de la saga)
  const renameSaga = async () => {
    const current = sagaHuman || sagaCanonical || "";
    const nextRaw = window.prompt("Nouveau nom de saga :", current);
    if (nextRaw == null) return; // annulé

    const nextCanonical = normalizeSaga(nextRaw);
    if (!nextCanonical) {
      toast({
        title: "Nom invalide",
    ...
    if (nextCanonical === sagaCanonical) {
    ...
    try {
      await Promise.all(
        games.map((g) =>
          updateGame(g.id, {
            ...g,
            // on stocke en version normalisée
            saga: nextCanonical,
          })
        )
      );
      toast({ title: `Saga renommée en “${nextCanonical}”` });

      // Redirection vers la nouvelle route de la saga
      const nextSlug =
        nextCanonical === SANS_SAGA_NAME ? SANS_SAGA_SLUG : slugify(nextCanonical);
      navigate(`/s/${nextSlug}`, { replace: true });
    } catch (e: any) {
      toast({
        title: "Échec du renommage",
        description: e?.message || "Impossible de renommer la saga.",
        variant: "destructive",
      });
    }
  };

  // 2) Déplacer tous les jeux vers “JEUX” (supprime la saga en pratique)
  const moveAllToJeux = async () => {
    if (
      !confirm(
        `Déplacer tous les jeux de “${sagaCanonical}” vers la section ${SANS_SAGA_NAME} ?`
      )
    )
      return;
    ...
    try {
      await Promise.all(
        games.map((g) =>
          updateGame(g.id, {
            ...g,
            // retirer la saga => ira dans “JEUX”
            saga: undefined,
          })
        )
      );
      toast({ title: `Jeux déplacés vers “${SANS_SAGA_NAME}”.` });
      navigate(`/s/${SANS_SAGA_SLUG}`, { replace: true });
    } catch (e: any) {
      toast({
        title: "Échec du déplacement",
        description: e?.message || "Impossible de déplacer les jeux.",
        variant: "destructive",
      });
    }
  };

  // 3) Supprimer tous les jeux de la saga (irréversible)
  const deleteWholeSaga = async () => {
    if (
      !confirm(
        `Supprimer DÉFINITIVEMENT tous les jeux de la saga “${sagaCanonical}” ?`
      )
    )
      return;
    ...
    try {
      ...
      toast({ title: "Saga supprimée (tous les jeux ont été supprimés)." });
      navigate(-1);
    } catch (e: any) {
      toast({
        title: "Échec de la suppression",
        description: e?.message || "Impossible de supprimer la saga.",
        variant: "destructive",
      });
    }
  };

  // ------- Ajout / Édition d’un jeu -------
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={renameSaga}>
                  Renommer la saga…
                </DropdownMenuItem>
                <DropdownMenuItem onClick={moveAllToJeux}>
                  Déplacer tous les jeux vers “{SANS_SAGA_NAME}”
                </DropdownMenuItem>
                <DropdownMenuItem
                  className="text-destructive focus:text-destructive"
                  onClick={deleteWholeSaga}
                >
                  Supprimer tous les jeux de cette saga
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
        ...
        <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <GameForm
              ...
              availableSagas={[]} // on force la saga actuelle
            />
          </DialogContent>
        </Dialog>
